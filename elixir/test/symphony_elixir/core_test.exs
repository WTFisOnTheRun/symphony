defmodule SymphonyElixir.CoreTest do
  ...
  test "active retry continues when needs-human ledger was resolved by a Goal Contract description update" do
    ...
    previous_log_file = Application.get_env(:symphony_elixir, :log_file)
    logs_root = Path.join(System.tmp_dir!(), "symphony-elixir-ledger-retry-description-#{System.unique_integer([:positive])}")
    ledger_root = Path.join(logs_root, "ledger")
    File.mkdir_p!(ledger_root)
    Application.put_env(:symphony_elixir, :log_file, SymphonyElixir.LogFile.default_log_file(logs_root))
    ...
    issue_id = "issue-ledger-retry-description"
    retry_token = make_ref()
    orchestrator_name = Module.concat(__MODULE__, :LedgerRetryDescriptionOrchestrator)

    File.write!(
      Path.join(ledger_root, "MT-561.json"),
      Jason.encode!(%{
        state: "NEEDS HUMAN",
    ...
    issue = %Issue{
      ...
      identifier: "MT-561",
      state: "Todo",
      title: "Needs human retry with updated contract",
      description: "Updated Goal Contract resolves the prior blocker.",
