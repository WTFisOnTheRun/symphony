defmodule SymphonyElixirWeb.Presenter do
  ...
  def issue_payload(issue_identifier, orchestrator, snapshot_timeout_ms) when is_binary(issue_identifier) do
    case Orchestrator.snapshot(orchestrator, snapshot_timeout_ms) do
      %{} = snapshot ->
        ...
        blocked = Enum.find(Map.get(snapshot, :blocked, []), &(&1.issue_identifier == issue_identifier))
  ...
  defp issue_payload_body(issue_identifier, running, retry, blocked) do
    %{
      ...
      last_error: (retry && retry.error) || (blocked && (blocked.error || blocked.blocker_reason)),
      tracked: %{}
    }
  end
  ...
  defp issue_status(nil, nil, blocked) when not is_nil(blocked), do: "blocked"
  ...
  defp blocked_entry_payload(entry) do
    %{
      ...
      last_event_at: iso8601(entry.last_event_at),
      last_message: summarize_message(entry.last_message),
      error: entry.error,
      blocker_reason: entry.blocker_reason,
      blocker_fingerprint: entry.blocker_fingerprint,
  ...
  defp blocked_issue_payload(blocked) do
    %{
      ...
      state: blocked.state,
      phase: blocked.phase,
      started_at: iso8601(blocked.started_at),
      completed_at: iso8601(blocked.completed_at),
      last_event: blocked.last_event,
      last_message: summarize_message(blocked.last_message),
      last_event_at: iso8601(blocked.last_event_at),
      error: blocked.error,
      blocker_reason: blocked.blocker_reason,
      blocker_fingerprint: blocked.blocker_fingerprint,
      next_human_action: blocked.next_human_action,
      tokens: %{
        input_tokens: blocked.codex_input_tokens,
        output_tokens: blocked.codex_output_tokens,
        total_tokens: blocked.codex_total_tokens
      }
    }
  end
